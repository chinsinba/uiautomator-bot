...
public class UiAutomatorHelper {
    ...
    private static boolean supportsUiAutomator(AndroidDevice device) {
        String apiLevelString = device.getiDevice().getMonkeyDevice().getProperty("ro.build.version.sdk");
        int apiLevel;
        try {
    ...
    private static void getUiHierarchyFile(AndroidDevice device, File dst, IProgressMonitor monitor) {
        if (monitor == null) {
        ...
        try {
            ...
            device.getiDevice().getMonkeyDevice().executeShellCommand(command,
        ...
        try {
            device.getiDevice().getMonkeyDevice().executeShellCommand(
                    command,
                    new CollectingOutputReceiver(commandCompleteLatch),
            ...
            device.getiDevice().getMonkeyDevice().getSyncService().pullFile(UIDUMP_DEVICE_PATH,
    ...
    public static UiAutomatorResult takeSnapshot(AndroidDevice device, IProgressMonitor monitor)
